...
export class SpiritDropdownManager {
    ...
    createSpiritItem(spirit) {
        ...
        a.querySelector('.spirit-icon').addEventListener('click', (e) => {
            e.preventDefault();
            this.selectSpirit(spirit.id);
        });
        
        // Link to Spirit detail page
        a.querySelector('.spirit-info-block').addEventListener('click', (e) => {
            e.preventDefault();
            window.location = '/spirit/' + spirit.id;
        });
