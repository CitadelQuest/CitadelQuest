...
export class CitadelExplorer {
    ...
    renderShares(shares, container) {
        ...
        shares.forEach(share => {
            ...
            // Content preview (if show_share_content is enabled)
            if (this.profile.show_share_content && share.preview_type) {
                if (share.preview_type === 'image' && share.preview_url) {
                    html += `
                        <div class="mt-3">
                            <img src="${share.preview_url}" alt="${share.title || ''}"
                                 class="rounded w-100" style="max-height: 500px; object-fit: contain; background: rgba(0,0,0,0.2);">
                        </div>`;
                }
                ...
                if (share.preview_type === 'text' && share.preview_content) {
                    ...
                    if (['md', 'markdown'].includes(ext)) {
                        ...
                        html += `
                            <div class="mt-3 p-3 rounded share-preview-scroll" style="background: rgba(0,0,0,0.2); max-height: 300px; overflow-y: auto;">
